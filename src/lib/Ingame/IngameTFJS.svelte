<script>
	...
	import {
		sleep,
		GetVideoStartAndEndTimeFromMetadata,
		GetScoringZoneEnabledAtTime
	} from '../utils';
	...
	$: {
		scoring_areas_component = $playGameMetadata.components.find(
	...
	const finishGame = () => {
		const startEndTime = GetVideoStartAndEndTimeFromMetadata($playGameMetadata);
	...
	const UpdateNumStars = (judgementTotals) => {
		...
		return GetNumStarsFromPerfectPercentage(GetPerfectPercentage(judgementTotals, scoringDuration));
	};
