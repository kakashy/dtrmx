...
<main>
	...
	{#if currentJudgement}
		<h2
			...
			style={`--judgement-color: ${JUDGEMENT_VISUALS[currentJudgement].color}; --judgement-frequency: ${JUDGEMENT_FREQUENCY}s`}
		>
...
<style>
	...
	div.background {
		...
		left: 0;
		width: 100%;
		clip-path: polygon(0% 0%, 100% 0%, calc(100% - var(--edge-pixels)) 100%, 0% 100%);
	...
	h2.judgement {
		...
		margin-left: 10px;
		--fallback-color: 'red';
		text-shadow: 0px 1px 2px rgba(0, 0, 0, 1), 0px 2px 4px rgba(0, 0, 0, 0.75),
	...
	div.star-container.filled {
		...
		width: 100%;
		height: 100%;
		--icon-filter: drop-shadow(0px 0px 1px rgba(0, 0, 0, 0.5))
			drop-shadow(0px 0px 2px rgba(0, 0, 0, 0.5)) drop-shadow(0px 0px 2px var(--color-yellow-light))
