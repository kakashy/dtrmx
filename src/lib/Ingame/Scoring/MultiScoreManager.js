...
export const RecordIdsOnFrame = (pose_ids) => {
	...
	const new_ids = pose_ids.filter(
		(id) => !ORIGINAL_IDS.includes(id) && !Object.keys(ID_CORRELATION_MAP).includes(id)
	);
	...
	if (missing_original_ids.length > 0 && new_ids.length > 0) {
		...
		for (let i = 0; i < new_ids.length; i++) {
			...
			const original_id =
				typeof missing_original_ids[i] == 'undefined'
					? missing_original_ids[0]
					: missing_original_ids[i];
