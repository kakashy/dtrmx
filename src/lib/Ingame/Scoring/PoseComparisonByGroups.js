...
const GetShapeSimilarityWithLookback = (groups, currentTime, playGameKeypointsValue) => {
	...
	for (
		...
		time > currentTime - lookback_time_amount;
		time -= lookback_interval
	) {
		const videoFrameKeypoints = GetKeypointsForTime(playGameKeypointsValue, time);
...
const GetGroups_NoOutliersHighestGroups = (groups, currentTime, playGameMetadataValue) => {
	const all_similarity_scores = GetShapeSimilarityWithLookback(
		groups,
		currentTime,
		playGameMetadataValue
	);
	const all_weighted_avg_scores = GetWeightedAvgGroupsScores(all_similarity_scores);
	const all_similarity_scores_no_outliers = RemoveWeightedAvgOutliers(
		all_similarity_scores,
		all_weighted_avg_scores
	);
...
// Compares the current frame group scores to the past X frames group scores and returns the averge similarities no outliers
const GetGroups_NoOutliersAverage = (groups, currentTime, playGameMetadataValue) => {
	const all_similarity_scores = GetShapeSimilarityWithLookback(
		groups,
		currentTime,
		playGameMetadataValue
...
// Compares the current frame group scores to the past X frames group scores and returns the averge similarities no outliers
const GetGroups_NoOutliersUpperHalf = (groups, currentTime, playGameMetadataValue) => {
	const all_similarity_scores = GetShapeSimilarityWithLookback(
		groups,
		currentTime,
		playGameMetadataValue
	...
	const upper_half = reorganized
		.sort((a, b) => b.avg - a.avg)
		.slice(0, Math.ceil(reorganized.length / 2));
...
// Compares the current frame group scores to the past X frames group scores and returns the averge similarities no outliers
const GetGroups_NoOutliersTopScore = (groups, currentTime, playGameKeypointsValue) => {
	const all_similarity_scores = GetShapeSimilarityWithLookback(
		groups,
		currentTime,
