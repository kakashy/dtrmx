...
export const GetCurrentTopXLastScores = (all_scores, currentTime, num_scores) => {
	...
	const last_scores_no_outliers = last_scores.filter((s) => s > top_score_threshold);

	if (last_scores_no_outliers.length == 0) {
		return all_scores_values[all_scores_values.length - 1]?.overall;
	}

	return last_scores_no_outliers.reduce((a, b) => a + b) / last_scores_no_outliers.length;
};
