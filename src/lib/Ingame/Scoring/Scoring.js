...
const AnalyzePose = async (
	...
	if (!is_miss) {
		...
		const PoseComparisonFunc = POSE_COMPARISON_BY_GROUPS_FUNC.NoOutliersTopScore;
		const group_scores = PoseComparisonFunc(groups, currentTime, playGameKeypointsValue);
		if (!group_scores) {
		...
		const thisTimeScore = GetCurrentTopXLastScores(ingameRawScoresValue[player_id], currentTime);
		...
		for (const group_name in raw_score.groups) {
			scoresString += `   ${group_name}: ${raw_score.groups[group_name].toFixed(2)}`;
		}
	...
	const scoringCurrentTime = GetScoringCurrentTime(
		currentTime,
		startEndTime.start,
		startEndTime.end,
		{}
