...
<main in:fade out:fade>
	<div class="container">
		...
		<h2 in:fly={{ delay: 1000, y: 30, duration: 800 }}>
			Welcome! It looks like this is your first time here, so let's get you set up with a sweet
			username.
		</h2>
		<input
			in:fade={{ delay: 1600, duration: 600 }}
			placeholder="SuperDancer123"
			bind:value={input_value}
		/>
		<div
			class={`button ${
				validateUsername(input_value.trim()) && input_value.trim().length >= 3 ? 'visible' : ''
			}`}
...
<style>
	...
	div.button div.label {
		...
		background: linear-gradient(45deg, var(--color-blue-dark) 0%, var(--color-blue-light) 100%);
	...
	div.error {
		background-image: linear-gradient(45deg, var(--color-red-dark) 0%, var(--color-red-light) 100%);
		color: white;
		font-weight: 700;
		padding: 30px;
		width: 100%;
