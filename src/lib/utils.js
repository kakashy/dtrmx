...
export const ConvertDurationToNiceStringWithDecimal = (duration) => {
	...
	const seconds = Math.floor(duration % 60)
		...
		.toString()
		.padStart(2, '0');
	const decimal = ((duration % 1) * 100).toFixed(0).toString().padStart(2, '0');
...
export const GetKeypointsForTime = (keypoints, time) => {
	...
	const t =
		(time_val - keypoints_keys[start_index]) /
		(keypoints_keys[end_index] - keypoints_keys[start_index]);
	...
	return {
		keypoints: start_keypoints.keypoints.map((keypoint) => {
			const matching_end_keypoint = end_keypoints.keypoints.find(
				(_keypoint) => keypoint.name == _keypoint.name
			);
...
export const GetVideoStartAndEndTimeFromMetadata = (metadata) => {
	const in_out_component = metadata.components.find(
		(component) => component.type == COMPONENT_TYPE.VIDEO_IN_OUT_POINTS
	);
	return in_out_component
		? { start: in_out_component.in, end: in_out_component.out }
		: { start: 0, end: metadata.duration };
};

export const GetVideoPreviewTimesFromMetadata = (metadata) => {
	const component = metadata.components.find(
		(component) => component.type == COMPONENT_TYPE.PREVIEW_AREA
	);
	return component
		? { start: component.in, end: component.out }
		: { start: 0, end: MAX_PREVIEW_TIME };
};
...
export const MirrorKeypoints = (keypoints) => {
	...
	for (const timestamp in keypoints) {
		mirrored_keypoints[timestamp] = {
			...keypoints[timestamp],
			keypoints: SortKeypointsByName(
				keypoints[timestamp].keypoints.map((keypoint) => {
					return {
						...keypoint,
						x: 1920 - keypoint.x,
