<script>
	...
	import WaveSurfer from 'wavesurfer.js?client';
	...
	async function GetThumbnails(project) {
		...
		const video_url = dlManager.metaData[project.media_id]['video_url'];

		if (!video_url) {
			return;
		}

		const thumbnail_blobs = await getThumbnailsInParalell(video_url, (progress) => {
	...
	const getNumThumbnailsToDisplay = async () => {
		...
		for (let i = 0; i < num_thumbnails_to_display; i++) {
			const timestamp = $createProject.duration * (i / num_thumbnails_to_display);
	...
	$: {
		seekerProgressPercent = `${($createVideoCurrentTime / $createProject.duration) * 100}%`;
	}
...
<main bind:clientWidth={width} style={`--timeline-padding: ${timeline_padding}px`}>
	<div class="lines">
