import { ConvertDurationToNiceStringWithDecimal, GetRoundedTimeFromTime } from '../utils';
...
const GetAvgKeypointFrequency = (project) => {
	...
	return {
		...
		message: `Every second has an average of ${one_per_x_kp} frame${
			one_per_x_kp == 1 ? '' : 's'
		} with keypoint data.`,
		severity: severity,
...
const GetKeypointsUnderScoreThreshold = (project, keypoint_score_threshold) => {
	...
	return groups.map((group) => {
		...
		const start_time = ConvertDurationToNiceStringWithDecimal(xx(min_frame, project.fps));
		const end_time = ConvertDurationToNiceStringWithDecimal(xx(max_frame, project.fps));
...
const GetKeypointSpacingProblems = (project) => {
	// Get all frames with _will_ be used for scoring
	const total_scoring_frames = xx(project.duration, project.fps);
