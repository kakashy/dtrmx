<script>
	...
	import {
		GetScoringZoneEnabledNextKeyframe,
		GetScoringZoneEnabledPrevKeyframe
	} from '../../../utils';

	let component;
	$: {
		component = $createProject.components.find(
			(component) => component.type == COMPONENT_TYPE.SCORING_AREAS
		);
	}
	...
	const getRightOffset = (keyframe_time) => {
		let next_keyframe_time = GetScoringZoneEnabledNextKeyframe(
			keyframe_time,
			component.keyframes,
			true
		);
		...
		{#each Object.keys(component.keyframes) as keyframe_time}
			{#if shouldRenderKeyframeArea(keyframe_time)}
				<div
					...
					style={`--left-offset: ${getLeftOffset(keyframe_time)}; --right-offset: ${getRightOffset(
						keyframe_time
					)};`}
				/>
				<div class="section start" style={`--left-offset: ${getLeftOffset(keyframe_time)}`} />
				<div class="section end" style={`--right-offset: ${getRightOffset(keyframe_time)}`} />
			{/if}
		{/each}
	</main>
{/if}
