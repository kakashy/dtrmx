...
<BasicComponent title={COMPONENT_DATA[type].name}>
	<div class="content-container">
		<h2>Project Name</h2>
		<input
			bind:value={$createProject.project_name}
			on:input={metadataChanged}
		...
		<h2>Chart Title</h2>
		<input
			bind:value={$createProject.title}
			on:input={metadataChanged}
			placeholder={'Chart Title'}
		/>
		<h2>Song Artist</h2>
		<input
			bind:value={$createProject.song_artist}
			on:input={metadataChanged}
		...
		<Dropdown
			...
			selectedOption={$createProject.difficulty}
		/>
		<div class="hr" />
