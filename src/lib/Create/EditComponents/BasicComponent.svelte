...
<main class={`background`} class:selected>
	<div class={`content`} on:click={(e) => OnClick(e)} bind:this={contentElement}>
		...
		{#if selected}
			<div
				class="slot"
				transition:slide|local={{ duration: contentElement.clientHeight, easing: cubicOut }}
			>
				<slot />
			</div>
		{/if}
...
<style>
	main.background {
		...
		background-image: linear-gradient(45deg, var(--background-color1), var(--background-color2));
