<script>
	...
	const onClickNextKeyframe = () => {
		const next_keyframe_time = GetScoringZoneEnabledNextKeyframe(
			$createVideoCurrentTime,
			component.keyframes,
			true
		);
	...
	const onClickPrevKeyframe = () => {
		const prev_keyframe_time = GetScoringZoneEnabledPrevKeyframe(
			$createVideoCurrentTime,
			component.keyframes,
			true
		);
...
<BasicComponent
	...
	{component}
	{componentIndex}
	title={COMPONENT_DATA[type].name}
>
	<div class="content-container">
		<h2>Scoring Enabled</h2>
		<div>
			<Dropdown onChanged={onDropdownChanged} {selectedOption} {options} />
		</div>
