		...
		{:else}
			...
			<div class="details-container">
				{#if !card_data.error}
					...
					<h2>Length</h2>
					<h3>
						{card_data.duration == 0 ? '--' : ConvertDurationToNiceString(card_data.duration)}
					</h3>
					<div class="hr" />
					<h2>Visibility</h2>
				...
				{:else}
					<h3 class="red">
						This video is not available. Please delete this project and try again. Double check the
						YouTube link to ensure it is correct.
					</h3>
				{/if}
			</div>
...
<style>
	...
	main.background.error {
		background-image: linear-gradient(45deg, var(--color-red-dark), var(--color-red-light));
	}
