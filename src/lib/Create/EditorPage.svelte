<script>
	...
	import { createProjectUnsaved, createProject } from '../stores';
	...
	onMount(() => {
		...
		if ($createProject) {
			GetObjectFromDB($createProject.uuid, DB_TABLES.LOCAL_KEYPOINTS).then((object) => {
				if (object) {
...
<main in:fade|local={{ delay: 400, duration: 0 }} out:fly|local={{ x: 500, duration: 200 }}>
