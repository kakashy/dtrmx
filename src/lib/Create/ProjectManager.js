...
GetVideoMetadataFromYouTube;

class ProjectManager {
	constructor() {
		this.ensureProjectsInLocalStorage();

		this.projects = browser ? JSON.parse(localStorage.getItem(PROJECTS_LOCALSTORE_NAME)) : {};
	...
	ensureProjectsInLocalStorage() {
		if (browser && !localStorage.getItem(PROJECTS_LOCALSTORE_NAME)) {
	...
	saveProject(project) {
		...
		StoreObject(
			JSON.stringify(project.keypoints),
			project.uuid,
	...
	createNewProject(name, youtube_link) {
		...
		this.updateProjectsInLocalStorage();
		GetVideoMetadataFromYouTube(youtube_link)
			.then((video_metadata) => {
