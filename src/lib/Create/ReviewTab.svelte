<script>
	...
	const onMessage = (evt) => {
		...
		if (evt.origin != 'http://localhost:4280' && evt.origin != 'https://dancetime.io') {
			return;
		}
...
>
	<section>
		...
		{#if extensionInstalled}
			<div class="aa-grid">
				<h2>
					Keypoint Score Threshold: {keypointScoreThreshold[0].toString().padEnd(4, '0')}
				</h2>
				...
				{#if !$createAAInProgress}
					<div class="button" on:click={clickAnalyzeButton}>Start Automatic Analysis</div>
				{:else}
					<div class="button" on:click={clickStopAnalyzeButton}>Stop Automatic Analysis</div>
				{/if}
				<div class="aa-details">
					<h3 class="percent-complete">
						{(GetAnalysisCompletion(analysisCompletion, $createProject.keypoints) * 100).toFixed(
							0
						)}% Complete
					</h3>
	...
	<section>
		...
		<h2>Play your chart to test it out! This is required to publish your chart.</h2>
	</section>
</main>
...
<style>
	...
	div.slider-container {
		--range-slider: var(--color-gray-500); /* slider main background color */
		--range-handle-inactive: var(--color-gray-300); /* inactive handle color */
		--range-handle: var(--color-yellow-dark); /* non-focussed handle color */
		--range-handle-focus: var(--color-yellow-light); /* focussed handle color */
		...
		--range-float-text: var(--color-gray-1000); /* text color on floating label */
	}
