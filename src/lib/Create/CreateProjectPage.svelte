<script>
	...
	$: {
		create_new_link, create_new_title, (create_new_valid = ValidateCreateNewParams());
	}
		...
		<main class="background" transition:fly|local={{ y: 100, duration: 400, easing: cubicOut }}>
			<div class="content">
				...
				<div class="details-container">
					<h2>Project Name</h2>
					<input
						bind:this={projectNameElement}
						bind:value={create_new_title}
					...
					<h2>Video Link</h2>
					<input
						bind:value={create_new_link}
						placeholder="https://www.youtube.com/watch?v=pdsGv5B9OSQ"
					/>
					<h2
						class={`create-button ${create_new_valid ? '' : 'invalid'}`}
...
<main
	class:blurred={current_state == STATE.CREATE_NEW}
...
<style>
	...
	section.title-section h1 {
		...
		background: linear-gradient(45deg, var(--color-yellow-dark) 0%, var(--color-yellow-light) 100%);
	...
	div.projects-container {
		...
		overflow: auto;
		gap: 30px;
		grid-template-columns: repeat(auto-fill, minmax(var(--card-height), 1fr));
	...
	div.create-new-container main.background {
		...
		align-items: stretch;
		background-image: linear-gradient(45deg, var(--color-yellow-dark), var(--color-yellow-light));
		min-height: 200px;
		height: fit-content;
		border-radius: 20px;
