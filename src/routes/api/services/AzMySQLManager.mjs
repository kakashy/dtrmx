...
const DATABASE_NAME =
	process.env.NODE_ENV == 'production' ? process.env.MYSQL_DB_PROD : process.env.MYSQL_DB_DEV;
...
class AzMySQLManager {
	...
	async submitChartScoreForUser(chart_id, user_id, score, judgements, chart_version) {
		...
		try {
			const result = await this.pool.query(
				'INSERT INTO chart_scores (timestamp, chart_id, user_id, score, judgements, version) VALUES (?, ?, ?, ?, ?, ?)',
	...
	async tryGetPublishedChartDetails(chart_id) {
		...
		const result = await this.pool.query(
			'SELECT chart_id, title, song_artist, choreography, difficulty, last_edited, video_link, video_id, duration, visibility, version, tags, user_id FROM charts WHERE chart_id = ? LIMIT 1',
			[chart_id]
		);
		if (result[0]) {
			return result[0];
	...
	async tryGetPublishedChartDuration(chart_id) {
		...
		const result = await this.pool.query('SELECT duration FROM charts WHERE chart_id = ? LIMIT 1', [
			chart_id
		]);
		if (result[0]) {
	...
	async tryGetPublishedChartKeypoints(chart_id) {
		...
		const result = await this.pool.query(
			'SELECT keypoints FROM charts WHERE chart_id = ? LIMIT 1',
			[chart_id]
		);
		if (result[0]) {
	...
	async createNewUser(email, username) {
		...
		try {
			const result = await this.pool.query('INSERT INTO users (username, email) VALUES (?, ?)', [
				username,
				email
			]);
		...
		try {
			...
			if (!existing_chart) {
				...
				await this.pool.query(query, [
					...
					chart.visibility,
					chart.version,
					this.jsonToCompressedString(chart.tags),
					this.jsonToCompressedString(chart.components),
					this.jsonToCompressedString(chart.keypoints),
					user.user_id,
					chart.project_id
				]);
			} else {
				...
				await this.pool.query(query, [
					...
					this.jsonToCompressedString(chart.components),
					this.jsonToCompressedString(chart.keypoints),
					existing_chart.chart_id || null,
					existing_chart.project_id,
					user.user_id
				]);
			}
			...
			const result = await this.pool.query(
				'SELECT * FROM charts WHERE chart_id = ? AND user_id = ? LIMIT 1',
				[chart.chart_id, user.user_id]
			);
			chart = result[0];
	...
	async emailIsWhitelisted(email) {
		...
		const result = await this.pool.query('SELECT * FROM invited_emails WHERE email = ? LIMIT 1', [
			email
		]);
		if (result[0]) {
