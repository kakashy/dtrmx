...
const MakeNewToken = (args, exp_time) => {
	return jwt.sign({ ...args }, process.env.JWT_PLAY_SECRET_KEY, {
...
export const UploadUserPlayScore = (context, req) => {
	return new Promise(async (resolve, reject) => {
		...
		await azMySQLManager.submitChartScoreForUser(
			chart_id,
			user_id,
			score,
			judgements,
...
export const SendUserPlayToken = (context, req) => {
	return new Promise(async (resolve, reject) => {
		...
		context
			.cookie('playToken', token, { maxAge: token_duration, httpOnly: true, secure: true })
			.end();
		resolve();
	});
};
